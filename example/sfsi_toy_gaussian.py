...
1.  Solving a non-linear inverse problem where the data is a product
    of the state and the parameter.
2.  Using a BiLaplacian Prior for regularization.
3.  Reading a mesh from an external XDMF file.
4.  Computing the MAP point and a low-rank (Laplace) approximation
    of the posterior.
5.  Drawing samples from the prior and the approximate posterior.
"""
...
def run_inversion(
    ...
    outdir: Path = Path("results-sfsi-toy-gaussian"),
    ...
    # This PDE is linear with respect to 'u' (for a fixed 'm')
    ...
    # --- 6. (OPTIONAL) VERIFY THE MODEL (GRADIENT CHECK) ---
    ...
    with dlx.io.VTXWriter(
        msh.comm,
        outdir / "qpact_BiLaplacian_prior_np{0:d}_Prior.bp".format(nproc),
    ...
    if use_vtx:
        ...
        with dlx.io.VTXWriter(
            msh.comm,
            outdir / "pact_prior_Bilaplacian_samples_np{0:d}.bp".format(nproc),
        ...
        with dlx.io.XDMFFile(
            msh.comm,
            outdir / "pact_prior_Bilaplacian_samples_np{0:d}.xdmf".format(nproc),
            "w",
        ) as file:
