...
This variation uses a "VariationalRegularization" prior,
...
def run_inversion(
    ...
    outdir: Path = Path("results-poisson-dirichlet-reg-example"),
) -> Dict[str, Dict[str, float]]:
    """
    Executes the full end-to-end Bayesian inversion workflow.
    (Dirichlet BCs + VariationalRegularization Prior)
    ...
    # (Store a function copy for later visualization)
    ...
    # --- 6. DEFINE THE PRIOR (H1 Tikhonov Regularization) ---
    ...
    # --- 7. DEFINE THE FULL BAYESIAN MODEL ---
    # Combine PDE, Prior, and Misfit into a single Model object
    model = hpx.Model(pde, prior, misfit)

    # --- 8. (OPTIONAL) VERIFY THE MODEL (GRADIENT CHECK) ---
    ...
    # --- 9. SOLVE FOR THE MAP POINT (INVERSION) ---
    ...
    # --- 10. POST-PROCESS AND SAVE MAP RESULTS ---
    ...
    # (m_true_fun was already created in step 4)
    ...
    with dlx.io.VTXWriter(
        msh.comm,
        outdir / "poisson_Dirichlet_Variational_Regularization_prior_np{0:d}_Prior.bp".format(nproc),
    ...
    # --- 11. COMPUTE LOW-RANK HESSIAN APPROXIMATION ---
